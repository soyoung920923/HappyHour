			    ...
			    <div class="card align-middle">
			        ...
			        <div class="card-body">
						...
						<label class="sy-only">브레이크&nbsp;&nbsp;
							<input type="checkbox" name="store_break" value="0" id="store_break"/> 있음
						</label><br>
						<select class="form-control store-select" name="break_start" id="break_start" disabled>
							<option value="" disabled selected hidden>선택하세요</option>
							<c:forEach items="${timeSet}" var="time">
								<option value="${time}" <c:if test="${store.store_open eq time}"> selected </c:if>>${time}</option>
							</c:forEach>
						...
						&nbsp;~&nbsp;
						<select class="form-control store-select" name="break_end" id="break_end" disabled>
							<option value="" disabled selected hidden>선택하세요</option>
							<c:forEach items="${timeSet}" var="time">
								<option value="${time}" <c:if test="${store.store_close eq time}"> selected </c:if>>${time}</option>
							</c:forEach>
